...
path = r"C:\Users\vietl\Desktop\Git\Python\halo-lidar"
min = 0
max = 0.5
df = pd.read_csv(path + '/' + 'depo.csv', header=None)
# %%
...
ax.set_extent([5, 45, 50, 75])

cmap = cm.jet
norm = Normalize(vmin=min, vmax=max)
for i, file_ in enumerate(df[0]):
    file = path + '/' + file_
    hydata, pathdata, header, datetime, multiple_traj = load_hysplitfile(file)
    pathdata = np.array(pathdata[1])
    if pathdata[0, 2] < 1000:
        print('Something is wrong with the height')

    ax.plot(pathdata[:, 0], pathdata[:, 1], linewidth=2, color=cmap(norm(df[1][i])),
            transform=ccrs.PlateCarree())
fig.colorbar(plt.cm.ScalarMappable(cmap=cmap, norm=norm), ax=ax,
             ticks=np.linspace(min, max, 6))
ax.set_title('OMG so beautiful', size=22, weight='bold')
gridliner_shallow = ax.gridlines(crs=ccrs.PlateCarree(), draw_labels=True)
gridliner_shallow.xlabels_top = False
gridliner_shallow.ylabels_right = False
fig.savefig(path + '/map.png', bbox_inches='tight')
